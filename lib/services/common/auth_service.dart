...
import 'package:kena/view/auth/widgets/account_verification_sheet.dart';
...
class AuthService {
  ...
  static void gotoNewPassword(
    ...
    Navigator.pushNamed(
      context,
      arguments: {'email': email, 'otp': otp, 'is_user': isUser},
  ...
  Future<void> verifyForgotOtp({
    ...
    final data = {'email': email, 'otp': otp, 'is_user': isUser};
    try {
      ...
      if (!context.mounted) return;
      gotoNewPassword(context, email, otp, isUser);
    } catch (e) {
      if (!context.mounted) return;
      ErrorHandler.handle(context, e, serviceName: tag);
    }
  ...
  Future<void> resetPassword({
    ...
    try {
      ...
      debugPrint('[$tag] Reset Password Payload: $data');
